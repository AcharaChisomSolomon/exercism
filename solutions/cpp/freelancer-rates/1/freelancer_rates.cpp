...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    return std::ceil(apply_discount(daily_rate(hourly_rate) * DAYS_PER_MONTH, discount));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double total_budget = budget * ((100 + discount) / 100);
    double day_rate = daily_rate(hourly_rate);
    return std::floor(total_budget / day_rate);
}
